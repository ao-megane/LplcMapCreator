...
/*
	...
	if (CheckHitKey(KEY_INPUT_D)) D++; else D = 0;
	if (CheckHitKey(KEY_INPUT_S)) S++; else S = 0;
	if (CheckHitKey(KEY_INPUT_W)) W++; else W = 0;
	if (CheckHitKey(KEY_INPUT_A)) A++; else A = 0;
	if ((GetMouseInput() & MOUSE_INPUT_LEFT) != 0) L_CLICK++; else L_CLICK = 0;
...
void PrintInput(int Key[]) {
	...
	DrawFormatString(0, 0, WHITE, "W:%d", W);
	DrawFormatString(0, 40, WHITE, "A:%d", A);
	DrawFormatString(0, 80, WHITE, "S:%d", S);
	DrawFormatString(0, 120, WHITE, "D:%d", D);
	DrawFormatString(0, 160, WHITE, "PAUSE:%d", PAUSE);
	DrawFormatString(0, 200, WHITE, "mause:(%d,%d):%d", mouse.Getx(),mouse.Gety(),L_CLICK);
